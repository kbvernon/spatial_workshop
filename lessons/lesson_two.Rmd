...
subtitle: "Spatial and Attribute Operations"
date: '`r Sys.Date()`'
output:
...
select(state_data, NAME, median_income_15)
...
plot(state_data["median_income_15"], key.pos = NULL, main = NULL)
...
rand_ohs %>% 
  st_join(states[, c("REGION", "ttl__15")]) %>% 
  group_by(REGION) %>% 
  summarize(pop = sum(ttl__15)) %>% 
  plot(pch = 19, cex = 2)
